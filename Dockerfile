...
RUN go get bitbucket.org/Southclaws/samp-objects-api
WORKDIR /go/src/bitbucket.org/Southclaws/samp-objects-api
RUN dep ensure
RUN make static

FROM scratch
COPY --from=compile /go/src/bitbucket.org/Southclaws/samp-objects-api/samp-objects-api /bin/samp-objects-api
